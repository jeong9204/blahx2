...
import { InMessage, InMessageServer } from './in_messages';
import { InAuthUser } from '../in_auth_user';

const MEMBDER_COL = 'members';
...
async function post({
  ...
  const memberRef = Firestore.collection(MEMBDER_COL).doc(uid);
  await Firestore.runTransaction(async (transaction) => {
    let messageCount = 1;
    const memberDos = await transaction.get(memberRef);
    if (memberDos.exists === false) {
      throw new CustomServerError({ statusCode: 400, message: '존재하지않는 사용자' });
    }
    const memberInfo = memberDos.data() as InAuthUser & { messageCount?: number };
...
async function updateMessage({ uid, messageId, deny }: { uid: string; messageId: string; deny: boolean }) {
  const memberRef = Firestore.collection(MEMBDER_COL).doc(uid);
  const messageRef = Firestore.collection(MEMBDER_COL).doc(uid).collection(MSG_COL).doc(messageId);
